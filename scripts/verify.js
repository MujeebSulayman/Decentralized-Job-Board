...
async function main() {
	...
	if (!fs.existsSync(contractAddressFile)) {
		console.error('ERROR: contractAddress.json not found. Please deploy contracts first.');
		process.exit(1);
	}
	...
	if (!jobBoardAddress || !paymasterAddress) {
		console.error('ERROR: Contract addresses not found in contractAddress.json');
	...
	if (!process.env.BASESCAN_API_KEY && !process.env.ETHERSCAN_API_KEY) {
		console.error('ERROR: BASESCAN_API_KEY or ETHERSCAN_API_KEY not set in .env file');
	...
	if (implementationAddress) {
		...
		try {
			...
			if (errorMsg.includes('Already Verified') || errorMsg.includes('already verified')) {
				...
				console.error('ERROR: JobBoard Implementation verification failed:', errorMsg);
				console.log('');
			}
		}
	}
		...
		if (errorMsg.includes('Already Verified') || errorMsg.includes('already verified')) {
			...
			console.error('ERROR: JobBoard verification failed:', errorMsg);
			console.log('');
		}
	}
