	...
	useEffect(() => {
		const fetchJobDetails = async () => {
			try {
				...
				setCompanyJobs(
					employerJobs.filter(
						(j) => j.employer === jobData.employer && j.id !== jobData.id
					)
				);
			} catch (error) {
							...
							<div>
								...
								<div className='flex items-center space-x-2'>
									...
									<span className='text-gray-400 text-sm'>
										...
										{companyJobs.length + 1 === 1 ? 'job' : 'jobs'} posted
									</span>
								</div>
							</div>
						</div>
							...
							<div className='bg-gray-800/40 backdrop-blur-sm rounded-2xl p-8 border border-gray-700/50'>
								<div className='flex items-center justify-between mb-6'>
									<h2 className='text-xl font-semibold'>
										More Jobs at {job.orgName}
									</h2>
									<span className='text-gray-400 text-sm'>
										{companyJobs.length} other{' '}
										...
										<div
											...
											<div>
												...
												<div className='text-sm text-gray-400'>
													Posted{' '}
													{formatDistanceToNow(
														new Date(Number(otherJob.startTime) * 1000),
														{ addSuffix: true }
													)}
												</div>
											</div>
											<div className='flex items-center space-x-4'>
												<span className='text-gray-400'>
													${otherJob.minimumSalary} - ${otherJob.maximumSalary}
												</span>
							...
							<div className='bg-gray-800/40 backdrop-blur-sm rounded-2xl p-6 border border-gray-700/50'>
								<div className='space-y-4 mb-6'>
									...
									<div className='flex items-center justify-between text-gray-400'>
										...
										<span>
											{formatDistanceToNow(
												new Date(Number(job.expirationTime) * 1000),
												{ addSuffix: true }
											)}
										</span>
									</div>
								...
								<div className='space-y-4 text-gray-400'>
									...
									<div className='flex items-center justify-between'>
										...
										<span className='text-white font-mono text-sm'>
											{`${job.employer.slice(0, 6)}...${job.employer.slice(
												-4
											)}`}
										</span>
									</div>
