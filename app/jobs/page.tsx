    ...
    useEffect(() => {
        const fetchJobs = async () => {
            try {
                ...
                const activeJobs = fetchedJobs.filter(job => !job.expired && !job.deleted);
                setJobs(activeJobs);
                ...
                <div className="space-y-4 mb-8">
                    <div className="flex flex-col md:flex-row gap-4">
                        ...
                        <div className="flex gap-2">
                            ...
                            {(filters.search || filters.location !== 'all' || filters.type !== 'all' || filters.salary !== 'all') && (
                                <button
                                ...
                                <select
                                    ...
                                    onChange={(e) => {
                                        ...
                                        if (newLocation !== 'all') {
                                            ...
                                            filtered = filtered.filter(job => Number(job.workMode) === locationValue);
                                        }
                                        ...
                                        if (filters.type !== 'all') {
                                            ...
                                            filtered = filtered.filter(job => Number(job.jobType) === typeValue);
                                        }
                                        ...
                                        if (filters.salary !== 'all') {
                                            const getSalaryRange = (salaryFilter: string) => {
                                                switch(salaryFilter) {
                                                    case '0-50k': return { min: 0, max: 50000 };
                                                    case '50k-100k': return { min: 50000, max: 100000 };
                                                    case '100k+': return { min: 100000, max: Infinity };
                            ...
                            >
                                ...
                                <div className="mt-4 space-y-2">
                                    <div className="flex items-center text-gray-400">
                                        ...
                                        <span>
                                            {Number(job.workMode) === 0 ? 'Remote' : 
                                             Number(job.workMode) === 1 ? 'On-site' : 
                                             'Hybrid'}
                                        </span>
                                    </div>
                                    <div className="flex items-center text-gray-400">
                                        ...
                                        <span>
                                            {job.minimumSalary && job.maximumSalary ? 
                                                ...
                                                job.minimumSalary ? `From $${Number(job.minimumSalary).toLocaleString()}` : 'Competitive'}
                                        </span>
                                    </div>
                                    <div className="flex items-center text-gray-400">
                                        <BriefcaseIcon className="h-5 w-5 mr-2" />
                                        <span>
                                            {Number(job.jobType) === 0 ? 'Full-time' : 
                                             Number(job.jobType) === 1 ? 'Part-time' : 
                                             Number(job.jobType) === 2 ? 'Internship' : 
                                             Number(job.jobType) === 3 ? 'Freelance' : 
                                             'Contract'}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="mt-4 flex items-center justify-between">
                                    <span className="text-sm text-gray-500">
                                        Posted {new Date(Number(job.startTime) * 1000).toLocaleDateString()}
                                    </span>
                                    <span className="px-3 py-1 text-sm rounded-full bg-purple-500/20 text-purple-400">
                                        {new Date(Number(job.expirationTime) * 1000) > new Date() ? 'Active' : 'Expired'}
                                    </span>
                                </div>
                            </div>
                        ))}
